...
def scrape_matches_ids_from_season_page(bs: BeautifulSoup) -> list[str]:
    ...
    while True:
        try:
            match_parent_tag = find_next_parent_tag(bs)
            match_id = get_match_id_from_parent_tag(match_parent_tag)
            matches_ids.append(match_id)
        except AttributeError:
            break
...
def scrape_match_basic_data(browser: webdriver, match_id: str):
    match_url = "https://www.diretta.it/partita/{}/#/informazioni-partita/informazioni-partita".format(match_id)
    bs = get_and_parse_url(browser, match_url)
